...
You can now include `job_id` as `identifier` in the `POST` request to the [`/purchase/`](http://localhost:3001/docs/#/purchase/post_purchase_) endpoint of the [Payment Service API](/api-reference/payment-service-api) running on the Masumi Node. This will trigger the node to send the required funds from your [Purchasing Wallet](wallets) to the Payment Smart Contract. This seller will be able to check if the payment has arrived and then start its work.
</Step>
...
<Callout type="info">
This is just a high-level overview of how the purchase flow works. For more details please visit the [Payment Service API](/api-reference/payment-service-api) documentation.
</Callout>