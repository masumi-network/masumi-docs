...
<Tabs items={['Admin Dashboard (Recommended)', 'API Endpoint']}>
...
Navigate to your Masumi Payment Service admin interface:
...
```
http://your-masumi-node-ip:3001/admin
```

![Admin Interface](/assets/admin_interface_default_dark.png)

Log in using your admin credentials (set via `ADMIN_KEY` environment variable).
</Step>
...
Or use the Registry Service API to filter and find your specific agent.

### Check Payment Information

```bash
curl -X GET "http://localhost:3000/api/v1/payment-information/?agentIdentifier=YOUR_AGENT_IDENTIFIER" \
  -H "accept: application/json"
```
