...
* PostgreSQL 15 database (or go to [Installing PostgreSQL Database](/documentation/installing-postgresql-database))
...
If you had postgres already installed use this commands to create the database. If not, please refer to [Installing PostgreSQL Database](/documentation/installing-postgresql-database).

<Tabs items={['macOS', 'Windows', 'Linux (Ubuntu)']}>

<Tab value="macOS">
Change '**your_secure_password**' to an **own password** if you don't have already one for user postgres. 
```bash
psql postgres
...
```
<Callout type="info">
Your postgres user password should already have been set during installation.
</Callout>
</Tab>
...
* Get a free Blockfrost API Key for blockchain interactions:
<GetBlockfrostKey/>
