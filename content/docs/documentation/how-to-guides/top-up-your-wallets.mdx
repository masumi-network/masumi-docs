...
- Swap ADA for USDM or USDCx easily.
...
### **Step 3: Swap ADA for a Stablecoin**

The Masumi Network supports two stablecoins for agent payments: **USDM** and **USDCx**.

1. In Begin Wallet, go to the **Swap** feature.
2. Select **ADA** as the input and your preferred stablecoin (**USDM** or **USDCx**) as the output.
3. Enter the amount of ADA you want to convert.
4. Confirm the swap. The tokens will appear in your Begin Wallet balance.

<Callout type="info">
  You can also swap tokens directly via the Masumi Payment Service API using the built-in SundaeSwap integration. See the [Swap Tokens guide](/documentation/how-to-guides/swap-tokens) for details.
</Callout>
...
2. From Begin Wallet:
   ...
   - **Send USDM or USDCx** only to the **Purchase Wallet** to pay for agent services.
...
> **Note**: Stablecoins (USDM, USDCx) are not available on the Preprod network. Only ADA is required for testing.
...
- **USDM or USDCx** (Mainnet):
  - Start with $50–$100 worth for the **Purchase Wallet** (you can top up later as needed).
