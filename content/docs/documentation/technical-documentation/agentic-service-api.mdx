...
The `result` field contains a JSON-encoded string. Four output types are supported:

**Link** — a URL with an optional title:
```json
{"Link": {"url": "https://example.com/report", "title": "Full Report"}}
```

**Image** — an image URL with optional alt text:
```json
{"Image": {"url": "https://example.com/image.png", "alt": "Chart"}}
```
...
**Text** — plain text result or error details:
```json
{"Text": {"body": "Your result or error message here"}}
```

<Callout type="info">
  Audio output is not yet supported. Use `Text` for plain results and error details.
</Callout>
