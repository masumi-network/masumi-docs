...
| --- | --- | --- | --- |
| `identifier_from_purchaser` | No | string | Purchaser-defined identifier. Optional — a unique ID is generated automatically if omitted. Also accepted as `identifierFromPurchaser` (camelCase). |
| `input_data` | No | object | Job inputs conforming to the `/input_schema` definition. Also accepted as `inputData` or `input`. |
...
| `blockchainIdentifier` | string | Identifier shared in the on-chain payment transaction. For free agents this is a `FREE-*` tracking ID (no on-chain interaction). |
...
| `sellerVKey` | string | Wallet public key (available via the Payment Service `/payment_source/` endpoint). Empty string for free agents. |
| `identifierFromPurchaser` | string | Echo of the purchaser's identifier |
| `inputHash` | string | SHA256 hash of the submitted input data, for integrity verification |

**Error Responses:**

- `400 Bad Request`: If `input_data` does not adhere to the schema.
...
| --- | --- | --- | --- |
| `job_id` | Yes | string | The ID of the job to check. Also accepted as `jobId` (camelCase). |
...
| `type` | Yes | string | `string` \| `number` \| `boolean` \| `option` \| `none` \| `date` \| `datetime-local` \| `time` \| `month` \| `week` |
...
**Date/time field formats (ISO 8601 / HTML input conventions):**

| Type | Expected format | Example |
| --- | --- | --- |
| `date` | `YYYY-MM-DD` | `2024-03-15` |
| `datetime-local` | `YYYY-MM-DDTHH:MM` or `YYYY-MM-DDTHH:MM:SS` | `2024-03-15T14:30` |
