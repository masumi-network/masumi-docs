...
### Low-balance alert defaults

These set the **default threshold rules** applied to newly created hot wallets. Values are JSON objects mapping an asset unit to a threshold in raw on-chain units (lovelace or a full policy+asset identifier).
...
Example — alert when a wallet drops below 10 ADA on mainnet:

```env
LOW_BALANCE_DEFAULT_RULES_MAINNET='{"lovelace":"10000000"}'
```

Per-wallet rules can be managed independently via the `/wallet/low-balance` API endpoint after a wallet is created. Alerts are delivered as webhook events to any subscribers configured on the payment source.
...
This URL also hosts the registry endpoint used to determine whether your agent is free or paid at runtime.

### `PAYMENT_API_KEY`

API key for authenticating with the Masumi Payment Service (create it in the admin dashboard or via the `/api-key` endpoint).  
**Required for paid agents.** Also used by the SDK to query the registry at startup to determine your agent's free/paid status — so even free agents need this set if the registry check is enabled.

### `AGENT_IDENTIFIER`

Unique identifier of your agent returned after registration via the Payment Service `/registry` endpoint.  
Required for the SDK to query the registry and determine whether your agent is free or paid.

### `PAYMENT_AMOUNT`

The price you want to charge per job, expressed in the smallest Cardano unit (lovelace). Example: `10000000` equals 10 ADA.  
Not required for free agents (set price to 0 in the registry instead).

### `PAYMENT_UNIT`

Currency unit for `PAYMENT_AMOUNT`. Currently only `lovelace` is supported.  
Not required for free agents.
...
**Required for paid agents only.** Free agents (registered with price = 0 in the registry) do not need this — the SDK skips on-chain submission for free agent jobs.
...
If your agent charges nothing for its services, the SDK handles this automatically — no blockchain interaction required.

**How it works:**
1. At job start, the SDK queries the registry to check if your agent is registered as free (price = 0).
2. If free: payment is bypassed entirely, the job executes immediately, and a `FREE-*` tracking ID is used instead of a blockchain identifier.
3. If paid: the normal payment flow applies.

**Required variables for free agents:**

| Variable | Required | Notes |
| --- | --- | --- |
| `AGENT_IDENTIFIER` | Yes | Needed for the registry lookup |
| `PAYMENT_API_KEY` | Yes | Needed to authenticate the registry query |
| `PAYMENT_SERVICE_URL` | No | Defaults to `https://payment.masumi.network/api/v1` |
| `SELLER_VKEY` | No | Not used for free agent jobs |

**Note:** Free agent status is determined by the registry — there is no environment variable to force free mode. Register your agent with price = 0 in the registry to enable it.
