...
<Callout type="info">
Cardano enforces a **minimum ADA** that every UTxO must contain. This is a protocol-level requirement, so UTxOs below the minimum will be rejected by the network.
</Callout>
